...
void LoadSavedGame(const ST::string &saveName)
{
	...
	LoadMapScreenMessagesFromSaveGameFile(f, stracLinuxFormat, getDataFilesEncodingCorrector());
...
static void LoadSoldierStructure(HWFILE const f, UINT32 savegame_version, bool stracLinuxFormat)
{
	...
	for (UINT16 i = 0; i < TOTAL_SOLDIERS; ++i)
	{
		...
		if(stracLinuxFormat)
		{
			...
			ExtractSoldierType(Data, &SavedSoldierInfo, stracLinuxFormat, savegame_version, getDataFilesEncodingCorrector());
		}
		else
		{
			BYTE Data[2328];
